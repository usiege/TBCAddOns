...
function OptionsList_OnLoad(self, ...)
	if self:GetName() ~= "GUI_DropDown" then
		hack(self, ...)
	end
end

local tabFrame1 = CreateFrame("Frame", "GUI_DropDown", _G["GUI_OptionsFrame"], "OptionsFrameListTemplate")
tabFrame1:Hide()
...
function GUI:CreateDropdown(title, values, vartype, var, callfunc, width, height, parent)
	...
	local dropdown = CreateFrame("Frame", "GUI_DropDown" .. self:GetNewID(), parent or self.frame, "UIDropDownMenuTemplate")

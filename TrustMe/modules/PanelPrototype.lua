...
function PanelPrototype:CreateCreatureModelFrame(width, height, creatureid)
	local model = CreateFrame("PlayerModel", "GUI_Option_" .. self:GetNewID(), self.frame)
...
function PanelPrototype:CreateText(text, width, autoplaced, style, justify, myheight)
	print("create text")
	--self.frame:SetSize(width, myheight)
	local textblock = self.frame:CreateFontString("GUI_Option_" .. self:GetNewID(), "ARTWORK")
	...
	--textblock:SetHeight(myheight or self.frame:GetHeight())
	if autoplaced then
...
function PanelPrototype:CreateButton(title, width, height, onclick, font)
	local button = CreateFrame("Button", "GUI_Option_" .. self:GetNewID(), self.frame, "UIPanelButtonTemplate")
...
function PanelPrototype:CreateColorSelect(dimension, useAlpha, alphaWidth)
	local colorSelect = CreateFrame("ColorSelect", "GUI_Option_" .. self:GetNewID(), self.frame)
...
function PanelPrototype:CreateSlider(text, low, high, step, width)
	local slider = CreateFrame("Slider", "GUI_Option_" .. self:GetNewID(), self.frame, "OptionsSliderTemplate")
...
function PanelPrototype:CreateScrollingMessageFrame(width, height, insertmode, fading, fontobject)
	local scroll = CreateFrame("ScrollingMessageFrame", "GUI_Option_" .. self:GetNewID(), self.frame)
...
function PanelPrototype:CreateEditBox(text, value, width, height)
	local textbox = CreateFrame("EditBox", "GUI_Option_" .. self:GetNewID(), self.frame, "InputBoxTemplate")
...
function PanelPrototype:CreateLine(text)
	local line = CreateFrame("Frame", "GUI_Option_" .. self:GetNewID(), self.frame)
...
do
	...
	function PanelPrototype:CreateCheckButton(name, autoplace, textLeft, dbmvar, dbtvar, mod, modvar, globalvar, isTimer)
		...
		local button = CreateFrame("CheckButton", "GUI_Option_" .. self:GetNewID(), self.frame, "OptionsBaseCheckButtonTemplate")
		...
		if modvar then -- Special warning, has modvar for sound and note
			if isTimer then
				...
				if mod.Options[modvar .. "SWNote"] then -- Mod has note, insert note hack
					frame2 = CreateFrame("Button", "GUI_Option_" .. self:GetNewID(), self.frame, "UIPanelButtonTemplate")
...
function PanelPrototype:CreateArea(name)
	local area = CreateFrame("Frame", "GUI_Option_" .. self:GetNewID(), self.frame, "OptionsBoxTemplate")
	area.mytype = "area"
...
function PanelPrototype:Destroy()
	tremove(GUI.frameTypes[self.frame.frameType], self.frame.categoryid)
...
do
	...
	function GUI:CreateNewPanel(frameName, frameType, showSub, sortID, displayName)
		if frameType == '1' then
			frameType = 1
		...
		local panel = CreateFrame("Frame", "GUI_Option_" .. self:GetNewID(), _G["GUI_OptionsFramePanelContainer"])
		panel.mytype = "panel"
		panel.sortID = self:GetCurrentID()
		if DEBUG then
			print("panel id " .. self:GetCurrentID())
		end
		local container = _G["GUI_OptionsFramePanelContainer"]
		panel:SetSize(container:GetWidth(), container:GetHeight())
		panel:SetPoint("TOPLEFT", "GUI_OptionsFramePanelContainer", "TOPLEFT")
		...
		--	frameType = 2
		--end
		panel.categoryid = GUI.frameTypes[frameType or 1]:CreateCategory(panel, self and self.frame and self.frame.name)

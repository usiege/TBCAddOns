...
local GUI, DEBUG = GUI, DEBUG

local frame = CreateFrame("Frame", "GUI_OptionsFrame", UIParent)

local first_load = true
function frame:UpdateMenuFrame()
	local listFrame = _G["GUI_OptionsFrameList"]
	if not listFrame.buttons then
		return
	end
	local displayedElements = self.tab and GUI.frameTypes[self.tab]:GetVisibleTabs() or {}
...
function frame:ClearSelection()
	for _, button in ipairs(_G["GUI_OptionsFrameList"].buttons) do
		button:UnlockHighlight()
	end
end
...
function frame:DisplayFrame(frame)
	...
	local changed = GUI.currentViewing ~= frame
	if GUI.currentViewing then
		GUI.currentViewing:Hide()
	end
	GUI.currentViewing = frame
	_G["GUI_OptionsFramePanelContainerHeaderText"]:SetText(frame.displayName)
	...
	if mymax > 0 then
		...
		for _, child in ipairs({ frame:GetChildren() }) do
			if child.mytype == "area" then
				child:SetPoint("TOPRIGHT", "GUI_OptionsFramePanelContainerFOV", "TOPRIGHT", -5, 0)
			end
		end
	end
	frameHeight = 20
...
function frame:DeselectTab(i)
	_G["GUI_OptionsFrameTab" .. i .. "Left"]:Show();
	_G["GUI_OptionsFrameTab" .. i .. "Middle"]:Show();
	_G["GUI_OptionsFrameTab" .. i .. "Right"]:Show();
	_G["GUI_OptionsFrameTab" .. i .. "LeftDisabled"]:Hide();
	_G["GUI_OptionsFrameTab" .. i .. "MiddleDisabled"]:Hide();
	_G["GUI_OptionsFrameTab" .. i .. "RightDisabled"]:Hide();
	self.tabs[i]:Hide()
end

function frame:SelectTab(i)
	_G["GUI_OptionsFrameTab" .. i .. "Left"]:Hide();
	_G["GUI_OptionsFrameTab" .. i .. "Middle"]:Hide();
	_G["GUI_OptionsFrameTab" .. i .. "Right"]:Hide();
	_G["GUI_OptionsFrameTab" .. i .. "LeftDisabled"]:Show();
	_G["GUI_OptionsFrameTab" .. i .. "MiddleDisabled"]:Show();
	_G["GUI_OptionsFrameTab" .. i .. "RightDisabled"]:Show();
	self.tabs[i]:Show()
end
...
function frame:CreateTab(tab)
	...
	local i = #self.tabs + 1
	self.tabs[i] = tab
	local button = CreateFrame("Button", "GUI_OptionsFrameTab" .. i, self, "OptionsFrameTabButtonTemplate")
	...
	if i == 1 then
		...
		button:SetPoint("TOPLEFT", "GUI_OptionsFrameTab" .. (i - 1), "TOPRIGHT", -15, 0)
	end

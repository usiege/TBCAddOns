...
local function InitVariables_Once() -- {{{
    ...
    D.Revision = "f1c71cc"; -- not used here but some other add-on may request it from outside
    D.date = "2019-09-02T06:01:55Z";
    D.version = "2.7.6.3-7-gf1c71cc";

    if D.date ~= "@project".."-date-iso@" then
        -- 1567404115 doesn't work
...
function D:VersionWarnings(forceDisplay) -- {{{
    ...
    if DcrMaxTOC < T._tocversion then
        ...
        elseif time() - self.db.global.TocExpiredDetection > 3600 * 24 * 40 then -- if more than 40 days elapsed since the detection
            ...
            if time() - self.db.global.LastExpirationAlert > 48 * 3600 or forceDisplay then
                ...
                T._ShowNotice ("|cff00ff00Decursive version: 2.7.6.3-7-gf1c71cc|r\n\n" .. "|cFFFFAA66" .. L["TOC_VERSION_EXPIRED"] .. "|r");
    ...
    if (("2.7.6.3-7-gf1c71cc"):lower()):find("beta") or ("2.7.6.3-7-gf1c71cc"):find("RC") or ("2.7.6.3-7-gf1c71cc"):find("Candidate") or alpha then
        ...
        if self.db.global.NonRelease ~= "2.7.6.3-7-gf1c71cc" then
            self.db.global.NonRelease = "2.7.6.3-7-gf1c71cc";
...
T._LoadedFiles["DCR_init.lua"] = "2.7.6.3-7-gf1c71cc";
